...
describe('childSleepSchedule.add mutation', () => {
  ...
  it('should throw error if bed time range from > to', async () => {
    const input: SleepScheduleInput = {
      bedTime: {
        from: '21:00',
        to: '20:00'
    ...
    expect(res.errors?.length).toBe(1);
    expect(res.errors?.[0].message).toEqual('Time range is not valid');
    expect(res.errors?.[0].extensions).toEqual(jasmine.objectContaining({
      code: 'BAD_USER_INPUT'
    }));
  });

  it('should throw error if wake up time range from > to', async () => {
    const input: SleepScheduleInput = {
      bedTime: {
        from: '20:00',
        to: '21:00'
