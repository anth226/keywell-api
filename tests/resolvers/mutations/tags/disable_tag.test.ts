...
const TAG_DISABLE = gql`
  mutation DisableTag($id: ID!) {
    tags {
      disable(id: $id) {
        id
        enabled
...
describe('disable tag mutations', () => {
  const bTagData = [
    {
  ...
  it('returns true value', async () => {
    ...
    expect(res.data.tags.disable).toEqual(
    ...
    expect(res.errors).toBe(undefined);
    expect(user.disabled_tags.length).toBe(2);
    expect(user.disabled_tags).toContain(mongoose.Types.ObjectId(bTagCreatedArr[0].id));
  ...
  it('returns success even though the provide tagid is already disabled', async () => {
    const {mutate} = initServerWithHeaders(server, authorizedHeaders)
