...
import {DiagnosisModel} from '../../../src/db/models';
...
describe('knownDiagnoses queries', () => {
  const diagnosisData = {
  ...
  it('found diagnose name with empty queries', async () => {
  ...
  it('found diagnose name sensitive with queries in the prefix', async () => {
    const {query} = initServerWithHeaders(server, authorizedHeaders)
    const variables = {
      name: 'Abc',
    }
  ...
  it('found diagnose name case sensitive with queries in the middle', async () => {
    const {query} = initServerWithHeaders(server, authorizedHeaders)
    const variables = {
      q: 'test keywell'
    }
  ...
  it('found diagnose name case sensitive with queries in the suffix', async () => {
    const {query} = initServerWithHeaders(server, authorizedHeaders)
    const variables = {
      q: 'Is A string'
    }
  ...
  it('found diagnose name case sensitive & title ordered', async () => {
    ...
    const diagnosisCreated_2 = await DiagnosisModel.create({
      ...diagnosisData_2,
      user: tokenPayload.id
    })
    ...
    // delete 2nd diagnosesResolver after invoking queries
  ...
  it('found diagnose name case belonged to user & public diagnose', async () => {
    const {query} = initServerWithHeaders(server, authorizedHeaders)
    // this is public diagnosis because it doesn't have user
    const diagnosisData_2 = {
      name: 'aac test Keywell is a string',
    }
    const diagnosisDataPublic = await DiagnosisModel.create({
      ...diagnosisData_2
    })
  ...
  it('found diagnose name case belonged to user & not found diagnose of another user', async () => {
    const {query} = initServerWithHeaders(server, authorizedHeaders)
    // this is user2's diagnosis because of user existence
    const diagnosisData_2 = {
      name: 'aac test Keywell is a string',
    }
    const diagnosisDataPublicCreated = await DiagnosisModel.create({
      ...diagnosisData_2,
  ...
  it('found diagnose name case belonged to user & public diagnose & not found diagnose of another user', async () => {
    ...
    const diagnosisDataUser2Created = await DiagnosisModel.create({
      ...diagnosisDataUser2,
      user: tokenPayloadUser2.id
    })
